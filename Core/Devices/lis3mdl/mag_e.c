...
// reads registers using I2C
int32_t mag_e_read(void* handle, uint8_t reg, uint8_t* buf, uint16_t len)
{
	return (int32_t)
		HAL_I2C_Mem_Read(handle, LIS3MDL_I2C_ADD_L, reg | 0x80, 1, buf, len, LIS3MDL_TIMEOUT);
}

// writes to registers using I2C
int32_t mag_e_write(void* handle, uint8_t reg, uint8_t* buf, uint16_t len)
{
	return (int32_t)
		HAL_I2C_Mem_Write(handle, LIS3MDL_I2C_ADD_L, reg | 0x80, 1, buf, len, LIS3MDL_TIMEOUT);
}

// checks status, gets and converts mag output
// 0 success, 1-3 HAL status, -1 no data
