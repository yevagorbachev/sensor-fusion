...
{
	return (int32_t)
		HAL_I2C_Mem_Read(handle, LSM303AGR_I2C_ADD_MG, reg | 0x80, 1, buf, len, LSM303AGR_MG_TIMEOUT);
}

int32_t mag_i_write(void* handle, uint8_t reg, uint8_t* buf, uint16_t len)
{
	return (int32_t)
		HAL_I2C_Mem_Write(handle, LSM303AGR_I2C_ADD_MG, reg | 0x80, 1, buf, len, LSM303AGR_MG_TIMEOUT);
}
